    ...
    <section className="bg-surface py-16 lg:py-24" aria-labelledby="offers-title">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mb-10 lg:mb-12">
          ...
          <h2 id="offers-title" className="font-display font-bold text-3xl sm:text-4xl lg:text-5xl text-ink leading-[1.1] mb-4">
            {tc('offers.title')}
          </h2>
              ...
              <Reveal as="li" key={plan.key} delay={index * 90} className="h-full">
                <article
                  className={`flex h-full flex-col rounded-card p-8 ${
                    plan.featured
                      ? 'bg-canvas text-ink border-2 border-accent'
                      : 'bg-canvas text-ink border border-line'
                  }`}
                >
                  {plan.featured && (
                    <span className="mb-4 inline-flex self-start rounded-full bg-accent-wash px-3 py-1 text-xs font-semibold uppercase tracking-[0.15em] text-accent-dark">
                      {t('popular')}
                    </span>
                  )}
                  ...
                  <p className="mt-6 flex items-baseline gap-2">
                    <span className="font-display font-bold text-4xl sm:text-5xl leading-none tabular-nums">
                      {t(`plans.${plan.key}.price`)}
                    </span>
                    {priceNote && (
                      <span className={"text-sm text-secondary"}>{priceNote}</span>
                    )}
                  </p>
