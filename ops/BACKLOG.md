...
| C24 | **Deux clients sont nommés en production sans accord** : `TL Services` et `L'Artpéro`, publiés dans le HTML servi depuis le 04/09 par le bloc SEO, qui imprime la clé `client` de chaque fiche sans filtre. Découvert le 22/09 en vérifiant la production, pas en lisant le code. Accord de TL Services attendu sous peu ; celui de L'Artpéro reste à demander — et son nom figure aussi dans le **titre** de la fiche, donc le ré-anonymiser demanderait de la réécrire. Voir [ADR-006](decisions/ADR-006-nommer-nostalgie.md). | Légal | P1 | todo | — | non |
