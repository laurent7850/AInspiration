...
**Dernière mise à jour :** 22 septembre 2026 · session Claude Code (le formulaire de contact n'allait pas où le site le dit — il partait sur `divers@distr-action.com` alors que toutes les pages publient `info@ainspiration.eu` ; corrigé, **et l'envoi est passé du Gmail personnel de Laurent au SMTP Hostinger d'`info@`**, de sorte que la réponse d'un prospect atterrit maintenant dans la boîte que le relevé planifié ouvre déjà. Deux défauts trouvés en relisant les messages reçus, invisibles dans l'éditeur n8n : l'accusé envoyé au prospect était **signé « L'équipe Distr-action »**, et chaque email commençait par un **`=` parasite** — préfixe `==` au lieu de `=`. Cinq soumissions, dix messages relus, corps rendus compris)
