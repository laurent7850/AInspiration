...
| 8 | **Carnet de bord `ops/` — mémoire du projet entre sessions** | **Posé le 22/09**, branche `ops/carnet-de-bord`, **rien de commité**. `ops/CONTEXTE.md` porte désormais les chiffres d'AInspiration — l'équipe des dix spécialistes est passée d'un dépôt voisin à un plugin utilisateur (`direction@distraction`) et ses fiches sont génériques. `ops/BACKLOG.md` amorcé à 17 chantiers, tous tirés des sections 3 et 4 de ce fichier, aucun deviné (`grep TODO/FIXME` hors `node_modules` : zéro). Bloc `permissions` fusionné dans `.claude/settings.json`. **Trouvé en chemin : une clé OpenRouter `sk-or-v1-…` en clair dans `n8n-workflows/README.md`, 4 occurrences, fichier suivi par git, dépôt public** — non touchée, la révocation coupe les workflows qui s'en servent et la valeur resterait dans l'historique. | **Feu vert de Laurent pour commiter**, puis trancher la clé OpenRouter (`ops/APPROVALS.md`). Sept données restent `— à compléter` dans le contexte, toutes économiques ou réglementaires : coût de revient, marges, plafonds de dépense, registre art. 30, dates de l'EU AI Act. |
