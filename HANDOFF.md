...
**Dernière mise à jour :** 26 septembre 2026 · l'état courant est dans `ops/` (`ops/journal/2026-09-26.md` pour le détail). Ce jour-là : les alertes « dockerd emballé » de la nuit venaient d'un `docker logs` **oublié par mon propre diagnostic** du 25/09 — le suiveur de log du démon tournait à 13 600 lectures/s, 1 à 2 cœurs pendant 17 h. Tué sans redémarrer le démon ; le watchdog nomme désormais lui-même le client accroché (C45). **Toute commande de diagnostic appelant `docker logs` se borne par `timeout`.** **Déployé le 26/09 et vérifié :** les corrections de contenu « irréprochable » sont en production dans les trois langues. C10 est quasi clos — l'offre d'**audit gratuit abandonnée le 16/09 était encore publiée aux moteurs** via un second jeu de questions en dur dans `HomePage.tsx`, donné à `getFAQSchema()` ; source unique posée dans `src/data/faq.ts`, générée depuis la locale FR. Et **Paperclip vivait dans quatre endroits, pas un** : le retirer de `src/data/realisations.ts` laissait le sitemap, la liste blanche du backend et les trois locales — le site le servait encore après le premier déploiement. **Supprimer une entité ici, c'est chercher son slug dans tout le dépôt, pas éditer la structure qui la porte.** Reste à ton arbitrage : les deux promesses « réponse sous 24h ». `health-check` 27/27, CI vert. **Audityo audite le meme jour, en production** : son accueil annoncait, a un public de PME, que la sanction retenue est « le plus eleve des deux » avant d'afficher 35 M€ — or l'**article 99 § 6 retient le plus BAS** pour une PME (60 000 € et non 15 M€ pour 2 M€ de CA). Le prompt du chatbot recopiait l'erreur du site : corrige dans n8n et teste. Le site ne se touche pas d'ici (une session par depot) — consigne prete dans . Voir C47.

**Mise à jour précédente :** 25 septembre 2026 (`ops/journal/2026-09-25.md`). En deux jours : cloisonnement OpenRouter terminé — sept projets, sept clés plafonnées à la journée, `Maudios` ne porte plus rien ; les **trois chatbots** lisent désormais le contenu réel de leur site à chaque message, et celui d'Audityo a été construit de zéro ; le prompt du chatbot AInspiration **inventait trois dirigeants, trois études de cas et des chiffres** — purgé ; TVA des mentions légales corrigée et déployée ; limitation de débit posée sur les webhooks de chatbot ; journal Traefik rendu silencieux.
...
2. **Un client est nommé dès que son nom est écrit dans la clé `client` des locales.**
   C'est le seul endroit où la décision se prend, et **rien ne la contrôle** : le bloc SEO
   du serveur (`routes/seo.js`, ligne 554) imprime cette clé pour chaque fiche, sans filtre,
   et c'est ce HTML que lisent les robots. Le champ `clientUrl` de `realisations.ts` ne rend
...
| 0b | **La sonde Audityo n'écrit plus dans la boîte d'Audityo** | **Fait le 19/09.** Nœud `IF` « Sonde ? » posé et publié dans `PeZexnxVbueaKV81` (version active `d19aada9`), entre `Message valide ?` et `Relayer vers la boîte` : tout e-mail différent de `sonde-audityo@surveillance.ainspiration.eu` passe, la sonde s'arrête là. La branche `CRM — Ingestion contact` est inchangée — c'est elle qu'on surveille, la sonde doit continuer de la traverser. `typeVersion: 2` **et** `conditions.options.version: 2` vérifiés dans la version publiée. La note fausse du nœud CRM (« ce webhook répond en mode lastNode ») a été corrigée au passage. | **22/09 : le trou n'était pas celui qu'on croyait, et il est bouché.** Le workflow n'avait **aucun `errorWorkflow`** et `saveDataErrorExecution: "none"` — un échec du nœud Gmail (credential expirée, quota, erreur d'API) n'alertait personne **et n'était même pas enregistré**. Posé : `errorWorkflow: qoHCxT04kuGtqRf7` et `saveDataErrorExecution: "all"`, confirmé par l'audit d'instance. Retour arrière = le réglage inverse, aucun nœud touché. **La question du 19/09 reste néanmoins ouverte :** `info@audityo.eu` est désormais lisible par API, mais ses deux seuls messages de formulaire datent du **15/09**, donc **avant** l'`IF` — et aucune soumission réelle n'est arrivée depuis. Il faut une soumission postérieure au 19/09. Tâche `handoff:ainspiration:verif-relais-mail-audityo` toujours pertinente. **Contrôle hebdomadaire conçu, un prérequis manque :** laisser la sonde traverser jusqu'à la boîte le lundi seulement, puis relire et supprimer le message par l'API — **même adresse de sonde**, donc la purge CRM existante la couvre déjà et `LeadSourceChart` n'est pas polluée. Prérequis : un token de l'API Email d'audityo.eu en credential n8n, **à créer par Laurent dans hPanel** (un token créé par API renverrait sa valeur dans une conversation). **Les trois autres workflows Audityo actifs ont été rattachés le 22/09 sur accord** (`Password Reset`, `Waitlist Welcome`, `Stripe Payment Alert Relay`), réglages propres à chacun conservés. Audit : 5 → 2 sans gestion d'erreur, et les 2 restants sont **inactifs**. **Deux trouvailles y attendent un arbitrage, aucune corrigée :** (1) leurs gardes `IF` portent `typeVersion: 2` sans aucun bloc `conditions.options`, donc sans `version: 2` — le piège de la section 5, ils laissent vraisemblablement tout passer. **Ne pas les « réparer » tels quels :** la condition de `Password Reset` exige un jeton à **deux** segments (`^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$`) là où un JWT en a **trois** — la réinitialisation marche vraisemblablement *parce que* le garde est inerte, et corriger la version sans la regex la casserait en production. (2) Le **webhook Stripe n'a aucune authentification** (champ `authentication` absent, là où les deux autres portent une `headerAuth`) et aucune **signature HMAC**, que la baseline exige pourtant sur les webhooks Stripe entrants : quiconque connaît l'URL déclenche un mail vers `divers@distr-action.com`. Ce workflow n'a **aucune exécution enregistrée** — jamais appelé, semble-t-il. |
| 0c | **Le formulaire de contact n'arrivait pas où le site le dit** | **Fait et vérifié le 22/09.** `wZuJtzIyeU4aVK4g`, nœud `Gmail - Send to Owner` : `divers@distr-action.com` → `info@ainspiration.eu`, alors que le footer, les mentions légales, la politique de confidentialité et les données structurées publient toutes `info@`. Dans la foulée et sur accord de Laurent, les deux nœuds `n8n-nodes-base.gmail` sont devenus des `n8n-nodes-base.emailSend` (typeVersion 2.1, `emailFormat: html`) sur une credential SMTP `smtp.hostinger.com:465` créée par Laurent : `messageId` passé de `@mail.gmail.com` à `@ainspiration.eu`. **Le gain n'est pas cosmétique** — la réponse d'un prospect à l'accusé arrive désormais dans `info@`. Deux défauts trouvés en relisant les corps reçus, invisibles dans l'éditeur : l'accusé au prospect était **signé « L'équipe Distr-action »** (copier-coller du workflow Distr'Action), et le champ portait `==` au lieu de `=`, donc **chaque email commençait par un `=` isolé**. Une régression introduite puis corrigée dans la session : `options` vidé avait réactivé l'attribution n8n (« sent automatically with n8n »), remise à `appendAttribution: false`. Cinq soumissions par le parcours réel, dix messages relus (uid 35→44), corps texte et HTML vérifiés pour les derniers ; le 200 du webhook n'a jamais servi de preuve. Gardes confirmés au passage : 400 sans `consent`, 429 au cinquième envoi. **La section « Messages » du CRM a été branchée dans la foulée** : `contact_messages` était **vide, zéro ligne**, alors que la route publique `POST /api/contact-messages` existait — personne ne l'appelait, le service frontend n'ayant pas de méthode de création et le workflow n'écrivant que dans `contacts`. Le dégât n'était pas la page vide mais le **compteur de notifications**, que `NotificationContext` alimente depuis `/contact-messages/stats` et qui ne pouvait donc jamais sonner. Nœud `CRM — Message de contact` posé sur la branche valide, en parallèle, `onError: continueRegularOutput` + timeout 5 s. Vérifié en base : `status: new`, propriétaire admin. **Et le menu CRM le signale enfin** : `CrmLayout.tsx` affiche un badge rouge sur « Messages » à partir de `newMessagesCount`, qui remplace le chevron quand il est présent. Déployé le 22/09 dans l'ordre — build 209 entrées, **push du manifeste d'abord**, `netlify deploy --prod`, **209/209 vérifiées une à une sur le CDN**, `--force-recreate`, conteneur reparti sur « Frontend: 209 files downloaded ». `bg-red-500` retrouvé dans le chunk `CrmLayout-CKAMp7Qg.js` servi en production ; asset absent toujours en 404 `text/plain`. Contrôle de santé : 24 passées, 1 échec — l'article du 16/09 sans `hreflang`, déjà suivi en 2b. | **Supprimer la fiche et la société de test** (`test-relais-info-22092026@ainspiration.eu`, contact `7cd4e873…`, société `185f4cb1…`), depuis `/contacts` **en admin** — elles sont invisibles en démo, par conception. **⚠️ La collecte POP3 de Gmail EST cassée, confirmé à 11h00 UTC** (`result: "Incorrect password"` à 10:51 depuis une adresse Google, après cinq relèves en succès le matin) — reporter le nouveau mot de passe dans Gmail → *Paramètres* → *Comptes et importation*, **sans attendre** : Gmail désactive la collecte après une série d'échecs. Contexte : le mot de passe de la boîte a été changé aujourd'hui (l'ancien était faux, prouvé par un `Incorrect password` SMTP depuis l'IP du VPS dans les journaux Hostinger), or Gmail relève `info@` en POP3 toutes les heures — sans mise à jour côté Gmail, la collecte s'arrête **en silence**. Restent : les noms du workflow et des nœuds disent encore « Gmail », et tous les autres workflows (`Error trigger`, les trois Audityo, les alertes VPS) envoient toujours par la credential Gmail. |
| 0d | **Les outils de communication se superposent** | **Cadré le 22/09, rien d'exécuté.** Audit en lecture seule des dépôts voisins, à la demande de Laurent : **4 outils** se disputent la publication sociale (CommunityOS, le module LinkedIn du CRM, The-event LinkedIn, DreamOracle RS) et **3** la publication d'articles (AutoSEO + 3 auto-blogs n8n + 3 « Publish from AutoSEO »). Trois faits tranchent : (1) le module LinkedIn du CRM **n'a jamais publié** — `linkedin_posts` rend 0 ligne, pour 1 263 lignes de code, 12 endpoints, un `client_secret` et un callback OAuth public à maintenir pour rien ; (2) le README de CommunityOS **se trompe** en disant qu'aucune plateforme réelle n'est contactée — son connecteur appelle bien `api.linkedin.com/rest/posts`, seul le déploiement (`ENABLED_CONNECTORS`) n'est pas activé ; (3) **le mur n'est pas dans les outils mais chez LinkedIn** — les trois publient `urn:li:person:`, et les pages entreprise exigent la Community Management API, soumise à revue. Changer d'outil ne débloque rien. Cible retenue : un outil par métier — social → CommunityOS seul, blog/SEO → AutoSEO seul, prospects → ce CRM, n8n → colle et jamais moteur. Analyse complète et consignes prêtes à coller dans `docs/chantiers/communication-multicanal.md`. | **Dans l'ordre, et rien avant.** (1) Laurent demande l'accès Community Management à LinkedIn — délai externe. (2) Session **CommunityOS** : obtenir **une** publication réelle ; tant qu'elle n'existe pas tout le reste est théorique. (3) **Ne pas toucher aux auto-blogs avant fin novembre** — la fenêtre de décision du 16/09 (500 impressions, 15 clics sur 28 jours) tient, l'avancer reviendrait à déplacer la barre après coup. (4) **Seule étape exécutable ici** : retirer le module LinkedIn du CRM — mais **après** que CommunityOS ait prouvé, jamais avant. |
...
| 7 | **n8n en 2.40.5 — l'image n'est pas épinglée** | **Monté le 22/09**, depuis la 2.35.4 du 19/08. `latest` et `stable` pointaient tous deux sur 2.40.5 ; la 2.41.0 existe mais n'est encore que `next`, elle n'a pas été prise. Sauvegarde à chaud **avant** bascule (`/usr/local/bin/n8n-backup.sh`, `n8n_20260922_112750.tar.gz`, 164 Mo, emportée par le `rclone` chiffré de 5h) : les migrations n8n ne sont pas réversibles, l'archive est le seul vrai retour arrière. Migrations du schéma passées sans erreur sur 1 Go de SQLite, `Recorded version change: 2.35.4 -> 2.40.5`. Vérifié après coup : éditeur 200 derrière Traefik, **32 workflows actifs** toujours actifs, **22 webhooks** réenregistrés, deux exécutions réussies dans la foulée, aucune erreur au boot. La clé de chiffrement des credentials vit dans `/root/n8n_test_data/config`, sur le bind-mount : elle a survécu au recreate, c'est ce qui rend les credentials encore lisibles. Ancienne image retaguée `n8nio/n8n:2.35.4-rollback` pour qu'un `docker image prune` ne l'efface pas. | **Épingler la version dans `/root/docker-compose.yml`** (`n8nio/n8n:2.40.5` au lieu de `latest`) : tel quel, un recreate déclenché pour autre chose montera silencieusement de version — y compris vers la 3.x quand elle sortira, avec ses migrations irréversibles et sans sauvegarde préalable. **Dépréciations annoncées au boot**, à traiter avant la v3 : `WEBHOOK_URL` → `N8N_WEBHOOK_URL` ; `binaryData` → `storage` (`N8N_MIGRATE_FS_STORAGE_PATH=true`) ; runner de tâches interne → externe ; trois plafonds dont le défaut baissera (timeout de tâche, décompression, entrées de zip). Et le compose porte encore `N8N_BASIC_AUTH_*`, **supprimées de n8n depuis la 1.0** : sans effet, mais elles laissent croire à une protection qui n'existe pas. |
| 8 | **Carnet de bord `ops/` — mémoire du projet entre sessions** | **Posé et fusionné dans `main` le 22/09** ([#43](https://github.com/laurent7850/AInspiration/pull/43)). `ops/CONTEXTE.md` porte désormais les chiffres d'AInspiration — l'équipe des dix spécialistes est passée d'un dépôt voisin à un plugin utilisateur (`direction@distraction`) et ses fiches sont génériques. `ops/BACKLOG.md` ouvre **22 chantiers** (C18 clos le jour même), dont six qui n'étaient suivis nulle part : double comptage GA4 depuis le 13/08, cloisonnement OpenRouter laissé en plan le 17/09, épuisement du pool de sujets vers février 2027, deux workflows « Publish from AutoSEO » jamais inspectés, et la chronologie de `/a-propos` qui contredit le `foundingDate` du JSON-LD. **Cinq ADR** sortent les décisions structurantes du tableau ci-dessus, qui n'en gardait que l'énoncé. Bloc `permissions` dans `.claude/settings.json` : `docker restart` refusé, `docker cp` soumis à confirmation. | **Une seule ligne attend un arbitrage, et elle est sérieuse** : une clé OpenRouter `sk-or-v1-…` **en clair** dans `n8n-workflows/README.md`, 4 occurrences, fichier suivi par git, **dépôt public**. Non touchée — révoquer coupe les workflows qui s'en servent encore, et la valeur resterait dans l'historique. Voir `ops/APPROVALS.md` et le chantier C3. Sept données restent `— à compléter` dans le contexte, toutes économiques ou réglementaires : taux journalier et marges (O1–O5 se vend en jours de Laurent, jamais chiffrés), registre art. 30, dates de l'EU AI Act. |
| 9 | **Le rendez-vous de découverte est réservable** | **Fait et déployé le 22/09.** `VITE_BOOKING_URL` était vide depuis toujours : les quatre boutons étaient masqués par `env.bookingUrl && (...)`, alors que ce rendez-vous est le **seul tunnel d'entrée** de la grille O1–O5. Cal.com hébergé (auto-hébergement sur le VPS écarté : un 44ᵉ conteneur pour une page de réservation). Trois corrections **avant** le build, la variable étant gravée dans le bundle : identifiant `divers-distr-action.com` → `ainspiration`, nom affiché « laurent Distraction » → la vraie marque, et adresse visant l'événement de 30 min plutôt que la page de profil, qui proposait aussi un créneau de 15 minutes. **Défaut trouvé en production seulement** : le bouton annonçait « 20 min » quand Cal.com, la grille et les CGV disent trente — six chaînes (3 langues × `common.json` et `local.json`) corrigées et redéployées. Deux déploiements, l'ordre tenu : manifeste poussé d'abord, 209/209 vérifiées sur le CDN, `--force-recreate`. Vérifié en production dans les trois langues ; 84/84 tests, contrôle de santé 21 passées / 1 échec (l'article du 16/09, chantier 2b). | **Ce qui reste appartient à Laurent** : la description du rendez-vous sur Cal.com, et le sort du créneau de 15 minutes — il existe toujours à son adresse, le site n'y renvoie pas. **Piège noté** : le build fait *avant* l'ajout de la variable était octet pour octet identique à la production (manifeste inchangé, `git status` vide). Un manifeste qui ne bouge pas après un build censé changer quelque chose est une information, pas un détail. |
